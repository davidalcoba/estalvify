...
describe("parseReportFilters", () => {
  ...
  it("maps the all-accounts sentinel to no account filter", () => {
    expect(parseReportFilters({ accountId: ALL_ACCOUNTS }, current, ["acc-1"]).accountId).toBe("");
    expect(parseReportFilters({ accountId: "acc-1" }, current, ["acc-1"]).accountId).toBe("acc-1");
  });

  it("drops an account the user does not have", () => {
    // A URL bookmarked before the account was deleted or deactivated, or one
    // pointing at somebody else's account. Falling back to all accounts beats
    // the alternative: a blank selector above an empty report.
    expect(parseReportFilters({ accountId: "gone" }, current, ["acc-1"]).accountId).toBe("");
    expect(parseReportFilters({ accountId: "acc-1" }, current, []).accountId).toBe("");
  });
});
