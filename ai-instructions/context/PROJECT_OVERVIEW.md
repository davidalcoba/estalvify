...
Dashboard & reports: the dashboard opens with a greeting picked from the hour in the MEMBER's timezone (`lib/greeting.ts`, bands cut at 06/14/21 where Spanish and Catalan cut them; the small hours are "evening", since English "Good night" is a farewell) — it was a fixed "Good morning" and read as unmaintained by mid-afternoon. Under it, two cards (`components/budget/weekly-card.tsx`, `control-mini.tsx`) side by side from `lg` up. **"To spend this week"** leads with the ISO week's operating number — the recalculated daily rate times the days left of the week — over the arithmetic that produced it (`3 days × 129,25 € a day`), the window it closes on (`Until Sunday`) and what is left for the rest of the month; under a rule, **"Spent this week"** carries the week's total, its operations counter against the 12-week median, and the composition by category. **"Categories"** is one line per discretionary objective: spent this week, and spent this month over its budget, toned by control state. Both cards state in a one-line description what their figures are — the previous version printed five unlabelled numbers and was unreadable to its own user (UI_RULES → "A derived figure shows its arithmetic"). Everything heavier — the cascade, the full objectives, the funds, the series — is one tap away on /plan. Reports show an income-vs-expenses trend, a spending-by-category donut, and top merchants, all driven by a filter bar (reference month, 6/12/24-month trend window, single bank account) whose state lives in the URL — `?month=YYYY-MM&trend=12&accountId=…` — so a filtered view is shareable and the cards re-suspend on their own while the filters stay usable. Trend/aggregation logic is in `lib/analytics/` (`spending.ts`, `trends.ts`, `report-filters.ts`); charts are theme-aware Recharts components in `components/reports/` using the `--chart-*` tokens.
