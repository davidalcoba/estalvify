...
Dashboard & reports: the dashboard leads with **Available to spend** — the month's variable budget (from the savings-first commitments, `lib/plan/month-status.ts`) minus variable spend (charges belonging to confirmed recurring series excluded via `splitVariableSpend`), with pace vs the point of the month and a per-day figure — plus real KPIs (net worth from latest balances, income this month with an income-concentration note when two holders earn, transactions to categorize) plus a 6-month income-vs-expenses chart and top categories; reports show an income-vs-expenses trend, a spending-by-category donut, and top merchants, all driven by a filter bar (reference month, 6/12/24-month trend window, single bank account) whose state lives in the URL — `?month=YYYY-MM&trend=12&accountId=…` — so a filtered view is shareable and the cards re-suspend on their own while the filters stay usable. Trend/aggregation logic is in `lib/analytics/` (`spending.ts`, `trends.ts`, `report-filters.ts`); charts are theme-aware Recharts components in `components/reports/` using the `--chart-*` tokens.

Plan (manual cash-flow planning): users declare the income and expenses they expect by hand, as standing `PlanItem`s (model in `prisma/schema.prisma`). Unlike the old month-by-month budget, a category can hold several items (e.g. rent monthly + car tax yearly) and each item has a cadence (`PlanCadence`: weekly / monthly / quarterly / yearly / one-off, with an optional day-of-month or a specific date). A periodic item can also carry an optional **end date** (`endDate`) for something with a term — a loan, a contract: it counts up to and including the month that date falls in and nothing after, so the forecast stops projecting it and its category limit drops. An ended item stays in the Plan, greyed out and labelled, so it can be extended or deleted rather than silently vanishing. The "does this still apply?" test is `isActiveInMonth`, and the steady-monthly helpers (`planTotals`, `plannedMonthlyByCategory`) take the reference month as an argument for exactly that reason. A category's steady monthly total is its **limit**, tracked against real spending with the same `ok/warning/over` progress model. Above the plan sits the **commitments card** (savings-first): fixed income − committed charges − the savings goal (Settings: fixed € or % of fixed income, treated as one more charge) − sinking-fund contributions = the month's variable budget. It also reports whether the savings transfer actually ran (TRASPASO/TRANSFER rows into the designated savings account) and the month's **real savings** as that account's net balance change. **Sinking funds** (`SinkingFund` model, pure math in `lib/plan/sinking-funds.ts`, card on /plan) provision monthly for foreseeable lumps; their accrual is computed, never cron-written. See `app/(app)/plan/`, `lib/plan/` (`plan-item.ts` pure logic + `plan-dto.ts`), and `components/plan/`. The pure `budget-progress`/`budget-dto` helpers in `lib/budget/` are reused for the limit bars. The old `/budget` route redirects to `/plan`; a migration carries each user's most recent monthly budget forward as monthly plan items.

Recurring payments / subscriptions: candidates are detected on the fly from the last ~13 months of transactions (grouped by a normalized merchant key, classified into weekly/monthly/quarterly/yearly cadences); the user confirms or ignores each, and decisions are stored in the `RecurringSeries` model (with a snapshot of cadence/amount for future forecasting and alerts). **Confirming a series adds it to the Plan automatically**: it upserts a `PlanItem` carrying the series' `merchantKey` in `recurringMerchantKey` (unique per user, so the link is 1:1 and re-confirming refreshes instead of duplicating), and ignoring or undoing the confirmation deletes that mirrored item again — auto-linked items only, never a hand-typed one. An expense with no category is the one case that is not planned (the Plan needs a category to build a limit); its row says so and offers a manual **"Add to Plan"** once categorized. Rows show an **"In Plan"** badge when linked, and the Plan marks mirrored entries with a repeat icon. The sidebar's Recurring item carries a **count badge of the series still awaiting a decision**, cached per user (`lib/recurring/review-count.ts`) because the app shell cannot rerun detection on every navigation. See `app/(app)/recurring/`, `lib/recurring/` (pure detector + DTO), and `components/recurring/`.

In-app notifications: a header bell surfaces alerts generated from the user's data — over/near a category's planned limit (from the Plan), upcoming confirmed recurring charges, a recurring charge that deviated >15% from its series' usual amount, an expected charge that never arrived, an extraordinary income to split and assign, the savings transfer not executed near month end, and low-balance alerts — monthly (plan-driven forecast) and **per-account daily** (cash-flow projection, with the covering transfer size). Generation is idempotent (upsert by `(userId, dedupeKey)` on the `Notification` model) and runs in the daily cron (`app/api/cron/sync`) plus an on-demand "Check now" action. Pure generators live in `lib/notifications/generators.ts` (with the impure gather/upsert in `generate.ts`); UI in `components/notifications/`. Push/email are future channels.

Forecast: the `/forecast` page projects the balance from the user's **Plan** — each future month's net comes from the planned income/expenses (`plannedForMonth`, with one-offs landing in their month), accumulated onto current net worth via `projectBalancesVariable`. When there is no Plan yet it falls back to the historical average net (last 6 full months) so the page still works day one. It also shows projected spend this month (linear extrapolation of actuals), average monthly net, and upcoming dated charges from the Plan (one-offs and monthly items with a day-of-month). Pure logic in `lib/analytics/forecast.ts` and `lib/plan/plan-item.ts`; the projected-balance area chart is `components/reports/balance-forecast-chart.tsx`. When the projection dips below zero it also emits the `LOW_BALANCE_PROJECTED` notification (`lib/notifications/generate.ts`, plan-driven, average fallback). Above the monthly view sits the **daily cash-flow projection** (next 60 days, per account and consolidated): confirmed recurring series scheduled on their real rhythm — month-end anchor for mortgage-style charges, median day for rent-style windows — plus a 90-day variable daily spend rate; per-account cards warn when a balance is set to dip under the user's threshold (Settings) before the salary lands. Pure math in `lib/analytics/cashflow.ts`, data assembly shared with the cron in `lib/analytics/cashflow-data.ts`.
...
Transaction splits & traceability: a bank row can be split into lines that must sum to its amount exactly (`TransactionSplit` model, editor in the transaction detail dialog, pure validation in `lib/transactions/splits.ts`). When lines exist they take over category aggregation; an income line can be flagged `isExtraordinary` (the April bonus inside the salary row), which subtracts it from every income average. Reports carries an **Untracked spending** card — ATM cash + monthly card settlements minus what splits explain (`lib/analytics/traceability.ts`) — because while that ~7% share is dark, every budget can look met while overspending. Accounts can carry a **holder** (`BankAccount.ownerName`, edited inline on /accounts) feeding the income-concentration indicator (`lib/analytics/household.ts`).
