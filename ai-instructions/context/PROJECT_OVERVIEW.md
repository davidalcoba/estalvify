...
Dashboard & reports: the dashboard opens with a greeting picked from the hour in the MEMBER's timezone (`lib/greeting.ts`, bands cut at 06/14/21 where Spanish and Catalan cut them; the small hours are "evening", since English "Good night" is a farewell) — it was a fixed "Good morning" and read as unmaintained by mid-afternoon. Under it, two cards (`components/budget/weekly-card.tsx`, `control-mini.tsx`) side by side from `lg` up, with **"Next charges"** (`upcoming-mini.tsx`) spanning both columns below them. **"To spend this week"** leads with the ISO week's operating number — the recalculated daily rate times the days left of the week — over the arithmetic that produced it (`3 days × 129,25 € a day`) and the window it closes on (`Until Sunday`). **The sign is a state, not a value**: once the month's variable budget is gone the card stops reporting a negative allowance and switches sentence — `0 €`, `248,16 € over the month's budget`, `Nothing left to spend until Sunday` — because a negative daily rate is not something anyone can act on (UI_RULES → "A sign flip is a different sentence"). Under the figure, a meter carries the month's spend against its budget with a mark where the calendar stands, so spending ahead of pace is a glance rather than a subtraction; under a rule, **"Spent this week"** carries the week's total, its operations counter against the 12-week median, and the composition by category, each row a proportional bar in the category's own colour (share of the week — no limits, no traffic lights). **"Categories"** is one line per discretionary objective: spent this week, and spent this month over its budget, toned by control state. Both cards state in a one-line description what their figures are — the previous version printed five unlabelled numbers and was unreadable to its own user (UI_RULES → "A derived figure shows its arithmetic"). **"Next charges"** is the forward half the weekly allowance needs — 47 € until Sunday means one thing with the month's charges paid and another with 600 € landing on Friday. It lists the next planned items over 14 days (date, description, amount; income in `success` with a `+`), with what is still to go out in the description. Two rules make it readable: the list is the FUTURE, so anything whose day has gone sinks below what is still to come; and a window that closed with nothing matched leaves the list for its own red line underneath, because sorting it by date buried the one row that asks for an action below the five-row limit. Selection is pure and unit-tested in `lib/planned/upcoming.ts`; `upcoming-data.ts` holds the single query, which rides on the `syncPlannedState` the screen already runs (the step it skips, `refreshSchedule`, writes only RecurringSeries fields nothing here reads). Everything heavier — the cascade, the full objectives, the funds, the series — is one tap away on /plan. Reports show an income-vs-expenses trend, a spending-by-category donut, and top merchants, all driven by a filter bar (reference month, 6/12/24-month trend window, single bank account) whose state lives in the URL — `?month=YYYY-MM&trend=12&accountId=…` — so a filtered view is shareable and the cards re-suspend on their own while the filters stay usable. Trend/aggregation logic is in `lib/analytics/` (`spending.ts`, `trends.ts`, `report-filters.ts`); charts are theme-aware Recharts components in `components/reports/` using the `--chart-*` tokens.
