...
Monthly control (/plan): the v2 cascade view — base income − planned charges − rollover-fund quotas − savings goal = the month's variable budget — plus the rollover funds card (quota + derived balance). The old `/budget` route redirects here. PlanItem and the old Plan UI were superseded by recurring series + planned items (see the planning-model paragraph below); `budget-progress`/`budget-dto` in `lib/budget/` survive for the notification path and legacy `get_budgets`.
...
Planning model v2: `planned_items` are the source of truth — recurring series (hand-maintained CRUD on /recurring, `RecurringSeries` with expectedAmount, day windows, month-end anchor, BIMONTHLY cadence and a matcher text) generate dated instances forward, one-offs are typed by hand, and a matching engine (`lib/planned/`) links arriving transactions (MATCHED with the actual amount; deviation >15% alerts) or flips closed windows to MISSED. The monthly cascade (`lib/budget/cascade.ts`, /plan "Monthly control") is base income (Settings, configuration — income above it is extraordinary by difference) minus planned charges minus rollover-fund quotas (`budget_items.rollover: true`, balance derived, auto-propagated month to month) minus the savings goal. The operating view is WEEKLY (`lib/budget/weekly.ts`, dashboard): a recalculated daily rate (no carry-over, no month-boundary special case), an operations counter against a 12-week median, and an informative composition. `/envelopes` labels the savings balance (stock, never mixed with the month's flow) and shows months of cushion. The daily cash-flow projection feeds from planned items.
