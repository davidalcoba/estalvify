...
- **Pull to refresh**: installed, there is no reload button, so the gesture is
  it. `components/layout/pull-to-refresh.tsx` renders the shell's `<main>` and
  `hooks/use-pull-to-refresh.ts` turns a downward drag at the top of any
  `(app)` screen into `router.refresh()` — a server re-render, not a page load,
  so client state survives. Touch devices only, and it takes over the
  browser's own overscroll rather than running beside it. Rules in
  `UI_RULES.md` → "Pull to refresh".
